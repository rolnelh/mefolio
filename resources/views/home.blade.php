...
<x-app-layout>
    ...
    @php
        ...
        $heroClusterGauche = $heroCreatifs->slice(0, 2)->values();
        $heroClusterDroit = $heroCreatifs->slice(2, 2)->values();
    @endphp
    ...
    <section class="relative bg-[#F7F6F1] py-24 sm:py-32 overflow-hidden">
        ...
        {{-- Grappes de portraits flottants, de part et d'autre du titre --}}
        @if ($heroClusterGauche->count())
            <div class="hidden md:block absolute top-[18%] left-[4%] lg:left-[10%] z-10">
                <div class="relative w-24 h-24">
                    @foreach ($heroClusterGauche as $j => $hc)
        ...
        @if ($heroClusterDroit->count())
            <div class="hidden md:block absolute top-[54%] right-[4%] lg:right-[10%] z-10">
                <div class="relative w-24 h-24">
                    @foreach ($heroClusterDroit as $j => $hc)
                        <a href="{{ route('creatifs.show', $hc->slug) }}"
                            class="absolute {{ $j === 0 ? 'top-0 left-0 z-10' : 'top-9 left-9 z-20' }} hover:z-30 hover:scale-110 transition-all duration-300">
                            <img src="{{ $hc->photo ?: 'https://ui-avatars.com/api/?name=' . urlencode($hc->prenom ?? 'M') . '&background=6366f1&color=fff' }}"
                                class="w-16 h-16 rounded-full object-cover shadow-xl ring-4 ring-[#F7F6F1]">
