...
<x-app-layout>
    ...
    {{-- Comment ça marche : cartes reliées par des flèches --}}
    <section class="bg-white py-24 px-6">
        <div class="max-w-6xl mx-auto">
            ...
            @php
                $steps = [
                    [
                        'num' => '1', 'title' => 'Créez votre profil',
                        'desc' => 'Renseignez votre spécialité, votre bio et votre portfolio en quelques minutes.',
                        'blob' => 'bg-indigo-50', 'iconColor' => 'text-indigo-500',
                        'icon' => 'M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z',
                    ],
                    [
                        'num' => '2', 'title' => 'Publiez vos projets',
                        'desc' => 'Montrez votre travail, recevez des likes et des commentaires de la communauté.',
                        'blob' => 'bg-violet-50', 'iconColor' => 'text-violet-500',
                        'icon' => 'M4.5 19.5l15-15m0 0H8.25m11.25 0v11.25',
                    ],
                    [
                        'num' => '3', 'title' => 'Soyez repéré',
                        'desc' => 'Postulez à des missions, grimpez le classement et faites-vous remarquer.',
                        'blob' => 'bg-emerald-50', 'iconColor' => 'text-emerald-500',
                        'icon' => 'M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z',
                    ],
                ];
            @endphp

            {{-- Desktop : cartes en ligne reliées par des flèches --}}
            <div class="hidden md:flex items-start justify-center gap-6">
                @foreach ($steps as $step)
                    <div class="w-64 bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg transition-all p-7 text-center">
                        <span class="w-16 h-16 mx-auto rounded-full {{ $step['blob'] }} flex items-center justify-center mb-5">
                            <svg class="w-7 h-7 {{ $step['iconColor'] }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $step['icon'] }}" />
                            </svg>
                        </span>
                        <span class="inline-block bg-amber-100 text-amber-900 text-sm font-bold px-4 py-1.5 rounded-full mb-3">
                            {{ $step['num'] }}. {{ $step['title'] }}
                        </span>
                    ...
                    @unless ($loop->last)
                        <svg class="w-12 h-12 text-amber-300 flex-shrink-0 mt-10" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 48 48">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4 28c6-14 16-14 20 0s14 14 20 0" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M38 22l6 6-6 6" />
                        </svg>
                    @endunless
                @endforeach
            </div>
            ...
            <div class="md:hidden space-y-4">
                @foreach ($steps as $step)
                    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 text-center">
                        <span class="w-16 h-16 mx-auto rounded-full {{ $step['blob'] }} flex items-center justify-center mb-4">
                            <svg class="w-7 h-7 {{ $step['iconColor'] }}" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $step['icon'] }}" />
                            </svg>
                        </span>
