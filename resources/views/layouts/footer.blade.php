    ...
    <div class="max-w-7xl mx-auto">
        <div class="bg-white border border-gray-100 rounded-[2rem] shadow-sm shadow-gray-900/[0.02] p-8 sm:p-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 pb-10">
                ...
                <div class="lg:col-span-4">
                    ...
                    <p class="text-sm text-slate-500 leading-relaxed mt-4 max-w-xs">
                        {{ __('La plateforme portfolio et marketplace des créatifs africains : designers, développeurs, photographes, vidéastes.') }}
                    </p>
                    ...
                    <form method="POST" action="{{ route('newsletter.store') }}" class="mt-6 max-w-xs">
                        ...
                        <div class="mt-2 flex items-center gap-1.5 bg-slate-50 border border-gray-200 rounded-full p-1.5">
                            <input id="footer-newsletter-email" type="email" name="email" required placeholder="{{ __('Votre email') }}"
                ...
                <div class="lg:col-span-2 lg:col-start-6">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('Produit') }}</h3>
                    <ul class="space-y-3 text-sm font-semibold text-slate-600">
                        <li><a href="{{ route('projects.index') }}" class="hover:text-indigo-600 transition-colors">{{ __('Projets') }}</a></li>
                        <li><a href="{{ route('creatifs.index') }}" class="hover:text-indigo-600 transition-colors">{{ __('Talents') }}</a></li>
                        <li><a href="{{ route('missions.index') }}" class="hover:text-indigo-600 transition-colors">{{ __('Missions') }}</a></li>
                ...
                <div class="lg:col-span-2">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('Communauté') }}</h3>
                    <ul class="space-y-3 text-sm font-semibold text-slate-600">
                        <li><a href="{{ route('classement.index') }}" class="hover:text-indigo-600 transition-colors">{{ __('Classement') }}</a></li>
                        <li><a href="{{ route('hackathons.index') }}" class="hover:text-indigo-600 transition-colors">{{ __('Programmes') }}</a></li>
                ...
                {{-- Ressources --}}
                <div class="lg:col-span-2">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('Ressources') }}</h3>
                    <ul class="space-y-3 text-sm font-semibold text-slate-600">
                        <li>
            ...
            <div class="pt-8 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3">
                <span class="text-[13px] text-slate-400 font-medium">{{ __('© :year Mefolio. Tous droits réservés.', ['year' => now()->year]) }}</span>
                <span class="text-[13px] text-slate-400 font-medium">{{ __("Fait avec soin par l'équipe Mefolio") }}</span>
            </div>
